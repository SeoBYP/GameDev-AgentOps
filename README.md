...
각 항목은 **구현한 것**과 그것이 보여주는 **역량**(→)을 함께 적었습니다.

**프레임워크 없이 짠 에이전트 루프** — 매 턴 `messages`를 키워 Claude를 호출하고, `stop_reason == "tool_use"`면 도구를 실행한 뒤 `tool_result`를 다음 user 턴으로 회신하는 루프를 직접 구현(`max_tokens`·무한루프 가드 포함). 응답은 `DownloadHandlerScript`를 상속해 SSE를 직접 파싱하며 토큰 단위로 표시(스트리밍).
→ LangChain·MAF 같은 에이전트 프레임워크에 의존하지 않고 **내부가 어떻게 도는지 이해**.

**멀티에이전트 위임** — `delegate` 도구의 *실행*이 곧 또 하나의 에이전트 루프(중첩 코루틴 `yield return RunAgent(...)`). sub-agent는 독립 세션·프로필에서 처리하고 **결론만** 반환해 메인 컨텍스트를 깨끗하게 유지(context 격리). sub엔 `delegate`가 없어 위임 깊이를 1단계로 제한.
→ 작업을 쪼개 위임·종합하는 **오케스트레이션 설계**.

**이중 권한 게이트** — (1) 모드별 허용 목록으로 도구를 필터(읽기 전용 모드는 쓰기 도구가 아예 보이지 않음) (2) 쓰기 도구는 코루틴을 멈추고 사용자 승인을 대기. 승인 정책(쓰기만 확인/전부 자동/전부 확인) 선택, 파일 도구는 경로 샌드박스를 거침.
→ AI에게 실행 권한을 주되 **안전하게** 통제(production sense).

**회복력 엔지니어링** — 429/529/5xx 같은 일시 오류는 지수 백오프로 재시도, 빈 메시지 가드로 잘못된 400 방지, 세션은 **도메인 리로드를 넘어 생존**하도록 영속.
→ 일시 오류·중단에도 끊기지 않는 **견고함**.

**엔진 통합 깊이** — UI Toolkit으로 채팅 UI를 짜고, `EditorCoroutine`로 비동기 처리, **리플렉션**으로 컴포넌트·타입을 동적으로 해석·조작.
→ Unity 에디터 API를 깊이 다룸.

**문제 해결 기록** — [문제→원인→해결 로그](docs/learning-journey.md)에 HTTP 상태코드 진단표, SDK 버전 불일치 대응, 코루틴/이벤트 처리 함정 등을 정리.
→ 막힌 지점을 원인까지 파고들어 해결한 흔적.
...
- Unity 로그 파서 · 데이터(CSV/밸런스) 검증 · 기획 문서 RAG · **MCP 공용 도구 계층**(JSON-RPC 2.0)으로 Unity/Unreal 공용화.
