...
| 구현 | 내용 |
|------|------|
| **에이전트 루프** | 매 턴 `messages`를 키워 호출 → `stop_reason == "tool_use"`면 도구 실행 후 `tool_result`를 다음 턴으로 회신 → 반복 (`max_tokens`·무한루프 가드). 응답은 `DownloadHandlerScript` 상속해 **SSE 직접 파싱**(스트리밍) |
| **멀티에이전트 위임** | `delegate` 실행이 곧 또 하나의 에이전트 루프(중첩 코루틴). sub-agent는 독립 세션에서 처리하고 **결론만** 반환(context 격리), 위임 깊이 1단계 제한 |
| **이중 권한 게이트** | ① 모드별 허용 목록으로 도구 필터(읽기전용 모드엔 쓰기 도구가 안 보임) ② 쓰기 도구는 코루틴을 멈추고 **승인 대기**. 승인 정책 선택 + 경로 샌드박스 |
| **회복력 엔지니어링** | 429/529/5xx는 **지수 백오프 재시도**, 빈 메시지 가드(400 방지), 세션은 **도메인 리로드를 넘어 생존**하도록 영속 |
| **엔진 통합 깊이** | UI Toolkit 채팅 UI, `EditorCoroutine` 비동기 처리, **리플렉션**으로 컴포넌트·타입 동적 해석·조작 |
| **문제 해결 기록** | [문제→원인→해결 로그](docs/learning-journey.md): HTTP 상태코드 진단표, SDK 버전 불일치 대응, 코루틴/이벤트 처리 함정 등 |
